...
function testSharedInterface(createClient) {
  ...
  describe('HTTP methods', () => {
    ...
    test('GET', () => {
      return server
        .captureRequest(() => sendRequest('GET'))
        .then(req => {
    ...
    test('POST', () => {
      return server
        .captureRequest(() => sendRequest('POST'))
        .then(req => {
    ...
    test('PUT', () => {
      return server
        .captureRequest(() => sendRequest('PUT'))
        .then(req => {
    ...
    test('PATCH', () => {
      return server
        .captureRequest(() => sendRequest('PATCH'))
        .then(req => {
    ...
    test('DELETE', () => {
      return server
        .captureRequest(() => sendRequest('DELETE'))
        .then(req => {
  ...
  describe('query params', () => {
    ...
    test('keys and values are both encoded', () => {
      ...
      return server.captureRequest(sendRequest).then(req => {
        expect(req.originalUrl).toContain(
          `?restaurant%20name=Wolf%20%26%20Man&access_token=${client.accessToken}`
        );
      });
    });
  });
  ...
  describe('paginated GET that succeeds for every page', () => {
    ...
    beforeEach(() => {
      server.setResponse((req, res) => {
        ...
        if (!req.query.start) {
          body = [{ a: 1 }, { a: 2 }];
          link = `<${server.origin}/tilesets/v1/mockuser?start=mockstart1>; rel="next">`;
        } else if (req.query.start === 'mockstart1') {
          body = [{ a: 3 }, { a: 4 }];
          link = `<${server.origin}/tilesets/v1/mockuser?start=mockstart2>; rel="next">`;
    ...
    beforeEach(() => {
      server.setResponse((req, res) => {
        if (!req.query.start) {
          res.append('Content-Type', 'application/json; charset=utf-8');
          res.append('Link', [
            `<${server.origin}/tilesets/v1/mockuser?start=mockstart1>; rel="next"`
          ]);
          res.json([{ a: 1 }, { a: 2 }]);
        } else if (req.query.start === 'mockstart1') {
          // Let it hang
